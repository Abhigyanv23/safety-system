...
export default function CameraPanel({ workerId }: { workerId: string }) {
  ...
  // Ensure simulated workers don't show broken hardware states
  ...
  // Resolve the URL to point to the backend port (5001) instead of Next.js (3000)
  ...
  return (
    <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-lg font-bold text-black dark:text-white">Camera</h2>
          <p className="text-xs text-gray-500 dark:text-slate-500 mt-1">OV5647 event snapshots</p>
        </div>
        <span 
      ...
      <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-slate-700 bg-gray-100 dark:bg-slate-900 min-h-[180px] flex items-center justify-center">
        {src ? (
          <img 
            src={src} 
            alt="Latest safety snapshot" 
            // Changed from object-cover to object-contain so critical incident details aren't cropped out
